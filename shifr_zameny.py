...
print("Reference freq analys of Russian alphabeth:" + str(fr_dict_podgon))  # reference
spis1 = ''.join(sorted_result.keys())
spis2 = ''.join(fr_dict.keys())
print("The freq analys like list:" + str(spis1))
print("The reference like list:" + str(spis2))
######################################################## encryption attempt
...
def entropy():
    ...
    with open('example.txt', encoding="utf-8") as ex:
        ...
        for keys in read_ex:
            result[keys] = result.get(keys, 0) + 1
        for k in result:
            pi[k] = (result.get(k, 0)) / len(read_ex)
    s = 0
    for value in pi.values():
        s += value
    print("Sum of probability: " + str(s))
    print("Probability of every symbol: ")
    for value in pi.values():
        if value > 0:
            h += float(value) * math.log((1 / float(value)), 2)
        print(value)
